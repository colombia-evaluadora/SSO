-- ============================================================================
-- V379 — mismo problema que V378, esta vez en CEVAL: "Por tablas" en
-- Auditoría mostraba "Sin tablas para mostrar" pese a que 162 de las 163
-- tablas de academico_test SÍ tienen el trigger trg_audit_ctx instalado.
-- Causa: /audit-tables/query armaba la lista con
-- `SELECT DISTINCT tabla FROM audit_log WHERE tabla LIKE 'academico_test.%'`
-- -- una tabla solo aparecía si YA tenía alguna operación auditada alguna
-- vez, y en este ambiente de test NINGUNA tabla de academico_test tiene
-- todavía ni una sola fila en audit_log (verificado en vivo). El ícono
-- fijo 'Table-Icon' tampoco resuelve a ningún ícono real del front (no
-- existe export "TableIcon" en el barrel -- toda tarjeta caía al genérico
-- de interrogación).
--
-- A diferencia de V378 (PIGSE, 18 tablas, catalogadas a mano una por
-- una): acá son 162. Catalogar cada una a mano no es proporcional, así
-- que el nombre visible se deja EXACTAMENTE con la misma fórmula
-- algorítmica de siempre (ya funcionaba razonablemente bien y nadie la
-- reportó como problema) y solo se reemplaza:
--   1. La FUENTE de la lista: en vez de derivar de audit_log, se
--      enumera la lista real y fija de tablas de academico_test con el
--      trigger instalado (162 tablas, menos tsesion_web -- esa es la
--      tabla de tracking de sesiones/login, no una entidad de dominio;
--      ya tiene su propia pantalla dedicada en Auditoría → Sesiones).
--   2. El ÍCONO: en vez del string fijo roto, se resuelve por
--      coincidencia de palabras clave en el nombre de la tabla (18
--      categorías cubren la gran mayoría; el resto cae a un ícono
--      genérico de documento, que al menos SÍ resuelve a algo real en
--      el front en vez de a QuestionIcon).
--
-- El slug NO se toca -- sigue la misma fórmula algorítmica que ya usa
-- /audit-tables/:SLUG/operations/query para reconstruir el nombre de
-- tabla a la inversa.
-- ============================================================================
...
DECLARE
    v_query_ok BOOLEAN;
BEGIN
    SELECT (q.query ILIKE '%academico_test.tfuncionario%' AND q.query ILIKE '%multiIf%') INTO v_query_ok
      FROM public.query q JOIN public.microservice m ON m.id_microservice = q.microservice_id
     WHERE m.serviceid = 'audit-clickhouse-cval' AND q.path_template = '/audit-tables/query';

    IF NOT coalesce(v_query_ok, false) THEN
        RAISE EXCEPTION 'V379: /audit-tables/query de audit-clickhouse-cval no se actualizo';
    END IF;

    RAISE NOTICE 'V379 OK: catalogo de /audit-tables/query (cval) enumera las 161 tablas reales de academico_test con icono resuelto por palabra clave.';
END $$;
