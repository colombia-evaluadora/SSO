    ...
    WITH universo AS (
        SELECT u.PK_TUNIDAD AS pk,
         ...
         WHERE (p_incluir_inactivos OR u.ACTIVE = TRUE)
           ...
           -- Fail-closed: si es docente puro y no se le resuelve funcionario,
           -- la comparacion con NULL descarta la fila.
...
BEGIN
    ...
    SELECT u.PK_TUNIDAD,
           ...
           -- Dependencia dinamica "referente -> rubrica" (V214.2), calculada
