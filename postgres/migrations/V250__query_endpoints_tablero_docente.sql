...
SELECT
    ...
    'V250 -- tarjetas del tablero "mis actividades" del DOCENTE autenticado (fn_actividad_resumen_estados_docente, V224/V250): pendientes_por_evaluar, en_evaluacion, finalizadas, vencidas, programadas, sin_programar y total, en una sola pasada. El docente se resuelve del token (fn_funcionario_actual sobre :CONTEXT.USER_ID) y NO es un parametro -- nadie puede pedir el tablero de otro docente por esta ruta. Solo cuenta actividades que el usuario DICTA (TDOCENTE_ASIGNATURA por grupo+asignatura), no las que aparecen en unidades de las que es autor. Filtros opcionales ?asignatura=, ?grupo=, ?unidad=, ?FECHA_DESDE=, ?FECHA_HASTA= y ?DIAS_GRACIA= (default 2, umbral de "Vencida"). Si el usuario autenticado no es un docente activo devuelve todos los contadores en 0 (no es error). Para el "Ver detalles" de cada tarjeta usar GET /planeador/actividades/mias?estados=... . Gate VER sobre PLANEADOR.'
...
SELECT
    ...
    '{"QUERY.SEARCH": "VARCHAR", "QUERY.ASIGNATURA": "BIGINT", "QUERY.GRUPO": "BIGINT", "QUERY.UNIDAD": "BIGINT", "QUERY.ESTADOS": "TEXT[]", "QUERY.DIAS_GRACIA": "INT", "QUERY.ORDEN_POR": "VARCHAR", "QUERY.ORDEN_ASC": "BOOLEAN", "QUERY.DIA": "DATE"}'::jsonb,
    'V250 -- "Ver detalles" de una tarjeta del tablero: pagina de las actividades del DOCENTE autenticado (fn_actividad_listar_docente, V224/V250), con las mismas columnas, orden y paginacion que GET /planeador/actividades. El docente se resuelve del token y NO es un parametro (nadie lista las actividades de otro docente por esta ruta); si el usuario no es un docente activo devuelve 0 filas. ?estados= es el filtro que viene de la tarjeta pulsada: array del estado DERIVADO (fn_actividad_estado) en {PENDIENTE_POR_EVALUAR, EN_EVALUACION, FINALIZADA, VENCIDA, PROGRAMADA, SIN_PROGRAMAR}; sin el, lista todas las del docente. ?DIAS_GRACIA= (default 2) define el umbral de VENCIDA, igual que en el tablero. PAGINADO POR DIA ACTIVO ?dia=YYYY-MM-DD (la barra "Hoy | MARTES 16 | < >" del tablero): deja solo las actividades VIGENTES ese dia -- vigente = la ventana [fechaInicio, fechaCierre] CUBRE el dia, no que empiece o cierre en el --, y devuelve dia / dia_anterior / dia_siguiente para las flechas (el dia ocupado mas cercano a cada lado, SALTANDO los vacios; NULL = no hay mas por ese lado). Sin ?dia= el listado es el de siempre. Filtros opcionales ?search=, ?asignatura=, ?grupo=, ?unidad=; orden ?ORDEN_POR= (whitelist fecha_inicio|fecha_cierre|fecha_creacion|titulo|ponderacion) / ?ORDEN_ASC=. IMPORTANTE: mandar ?size= y ?offset= SIEMPRE explicitos (bug conocido del motor query-service con los system-bound de paginacion). total_count via COUNT(*) OVER(). Gate VER sobre PLANEADOR.'
  FROM public.microservice m
 WHERE m.serviceid = 'eval-col'
ON CONFLICT (microservice_id, path_template, http_method) WHERE path_template IS NOT NULL DO NOTHING;
