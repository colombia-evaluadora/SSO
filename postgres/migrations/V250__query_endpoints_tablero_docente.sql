...
SELECT
    ...
    'V250 -- "Ver detalles" de una tarjeta del tablero: pagina de las actividades del DOCENTE autenticado (fn_actividad_listar_docente, V224/V250), con las mismas columnas, orden y paginacion que GET /planeador/actividades. El docente se resuelve del token y NO es un parametro (nadie lista las actividades de otro docente por esta ruta); si el usuario no es un docente activo devuelve 0 filas. ?estados= es el filtro que viene de la tarjeta pulsada: array del estado DERIVADO (fn_actividad_estado) en {PENDIENTE_POR_EVALUAR, EN_EVALUACION, FINALIZADA, VENCIDA, PROGRAMADA, SIN_PROGRAMAR}; sin el, lista todas las del docente. ?diasGracia= (default 2) define el umbral de VENCIDA, igual que en el tablero. PAGINADO POR DIA ACTIVO ?dia=YYYY-MM-DD (la barra "Hoy | MARTES 16 | < >" del tablero): deja solo las actividades VIGENTES ese dia -- vigente = la ventana [fechaInicio, fechaCierre] CUBRE el dia, no que empiece o cierre en el --, y devuelve dia / dia_anterior / dia_siguiente para las flechas (el dia ocupado mas cercano a cada lado, SALTANDO los vacios; NULL = no hay mas por ese lado). Sin ?dia= el listado es el de siempre. Filtros opcionales ?search=, ?asignatura=, ?grupo=, ?unidad=; orden ?ordenPor= (whitelist fecha_inicio|fecha_cierre|fecha_creacion|titulo|ponderacion) / ?ordenAsc=. IMPORTANTE: mandar ?size= y ?offset= SIEMPRE explicitos (bug conocido del motor query-service con los system-bound de paginacion). total_count via COUNT(*) OVER(). Gate VER sobre PLANEADOR.'
  FROM public.microservice m
 WHERE m.serviceid = 'eval-col'
ON CONFLICT (microservice_id, path_template, http_method) WHERE path_template IS NOT NULL DO NOTHING;
