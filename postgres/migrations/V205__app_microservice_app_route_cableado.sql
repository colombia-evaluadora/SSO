...
-- ---------------------------------------------------------------------------
-- 0. La app SSO-ADMIN - prerequisito de todo lo demas.
--
--    Mismo patron que el resto de este lote: la fila llego por el dump base y
--    ninguna migracion la crea. V14 hace
--        UPDATE app SET launch_url='/admin/' WHERE name='SSO-ADMIN'
--    dandola por existente, y V10 ata las rutas /admin/** a esa app por
--    nombre: sobre una base limpia ambas son no-ops silenciosos, y las 4
--    filas de app_microservice del paso 1 que apuntan a SSO-ADMIN tampoco se
--    insertarian.
--
--    launch_url = '/admin/' es exactamente lo que V14 intenta fijar y lo que
--    tiene el servidor. La descripcion tambien se copia de alli.
-- ---------------------------------------------------------------------------
INSERT INTO app (name, description, launch_url)
VALUES ('SSO-ADMIN', 'SSO Admin Console', '/admin/')
ON CONFLICT (name) DO NOTHING;


-- ---------------------------------------------------------------------------
-- 0.b Reaplicar los binds de V10, que se perdieron por el mismo motivo.
--
--     V10 ata la app SSO-ADMIN a su rol y a todas las rutas /admin/**, pero
--     resuelve la app por nombre y corre mucho antes que el paso 0: sobre una
--     base limpia no encontraba nada y no ataba nada. Crear la app sin esto
--     dejaria la consola de administracion registrada pero sin ninguna ruta,
--     que es peor que no tenerla.
--
--     Son las MISMAS dos sentencias de V10, con un solo cambio: el rol se
--     acepta como 'SSO-ADMIN' o 'ADMIN' porque el catalogo de roles esta
--     partido entre entornos (V36 paso 2 renombra a SSO-*, pero
--     DataInitializer de auth-center recrea ADMIN/USER sin prefijo despues de
--     migrar -- lo advierte el propio docblock de V36). Aceptar ambos evita
--     decidir esa discusion aqui.
-- ---------------------------------------------------------------------------
 ...
 WHERE a.name = 'SSO-ADMIN'
   ...
   AND NOT EXISTS (
       SELECT 1 FROM app_route ar
        WHERE ar.id_app = a.id_app AND ar.id_route = ro.id_route
   );
