-- ===========================================================================
-- V478 - El instrumento de evaluacion lo admite el REFERENTE, no la unidad.
...
BEGIN
    ...
    --     fn_actividad_instrumento_contexto_assert (V478), compartida con el PATCH.
...
UPDATE public.query q
   SET detail = q.detail || ' V478 -- La evaluacion (instrumento) ya no se apaga por no tener unidad: sale del referente curricular que le aplica a la actividad, que sin unidad se deriva de su grado y asignatura. En nivel Preescolar sigue apagada.'
  FROM public.microservice m
 WHERE m.id_microservice = q.microservice_id
   AND m.serviceid       = 'eval-col'
   AND q.path_template   = '/planeador/actividades/:ID/configuracion'
   AND q.http_method     = 'GET'
   AND q.detail NOT LIKE '%V478 --%';

UPDATE public.query q
   SET detail = q.detail || ' V478 -- El instrumento de evaluacion ya no exige unidad: se acepta cuando el referente que le aplica al contexto (unidad, o grado del grupo + asignatura) es EVALUATIVO, y se rechaza con 422 (22023) solo si es Formativo. Desvincular la unidad de una actividad que ya tenia instrumento tampoco falla.'
  FROM public.microservice m
 WHERE m.id_microservice = q.microservice_id
   AND m.serviceid       = 'eval-col'
