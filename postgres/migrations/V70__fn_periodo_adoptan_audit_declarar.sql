-- V70 — adopta fn_audit_declarar en las funciones de escritura de periodo
-- académico, descansos y periodo de evaluación
-- (docs/etiqueta-catalogo-funciones-fn.md §7/§8), mismo patrón que V67-V69.

CREATE OR REPLACE FUNCTION academico_test.fn_descanso_agregar(
    p_fk_periodo bigint, p_hora_inicio time without time zone, p_hora_fin time without time zone,
...
BEGIN
    ...
    PERFORM academico_test.fn_audit_declarar(
        ...
        format('Agregado de descanso %s-%s al periodo académico', p_hora_inicio, p_hora_fin),
...
RETURNS bigint LANGUAGE plpgsql AS $$
DECLARE
    v_audit VARCHAR(120) := p_pk_usuario_solicitante::VARCHAR; v_n INT;
    v_establecimiento_id BIGINT; v_hi TIME; v_hf TIME;
BEGIN
    IF NOT academico_test.fn_periodo_usuario_puede_gestionar(p_pk_usuario_solicitante) THEN
        RAISE EXCEPTION 'El usuario no tiene el nivel de permisos necesario para realizar esta accion'
            USING ERRCODE = '42501';
    ...
    SELECT s.FK_TESTABLECIMIENTO, d.HORA_INICIO, d.HORA_FIN
      INTO v_establecimiento_id, v_hi, v_hf
    ...
    PERFORM academico_test.fn_audit_declarar(
        p_pk_usuario_solicitante, format('Eliminación del descanso %s-%s', v_hi, v_hf), v_establecimiento_id);
...
CREATE OR REPLACE FUNCTION academico_test.fn_periodo_actualizar(
    p_pk_periodo bigint, p_fk_estado bigint DEFAULT NULL::bigint, p_fk_sede bigint DEFAULT NULL::bigint,
    p_fecha_inicio date DEFAULT NULL::date, p_fecha_fin date DEFAULT NULL::date,
    p_fecha_limite_matricula date DEFAULT NULL::date, p_fk_jornada bigint DEFAULT NULL::bigint,
    p_reserva academico_test.bool_sn DEFAULT NULL::character varying, p_bloques_por_defecto bigint DEFAULT NULL::bigint,
    p_fk_periodo_anterior bigint DEFAULT NULL::bigint,
...
BEGIN
    ...
    IF r.ACTIVE = FALSE THEN
        RAISE EXCEPTION 'El periodo academico % esta inactivo; no se puede actualizar', p_pk_periodo
            USING ERRCODE = '22023';
    END IF;
    -- El establecimiento no cambia dentro de esta funcion (se valida mas abajo
    -- que una sede nueva pertenezca al mismo establecimiento), asi que resolverlo
    -- una vez aqui sirve tanto para el gate como para la etiqueta al final.
    SELECT FK_TESTABLECIMIENTO INTO v_establecimiento_id FROM academico_test.TSEDE WHERE PK_TSEDE = r.FK_TSEDE;
    ...
    IF v_ano_id IS NULL THEN
        SELECT PK_ANO_LECTIVO INTO v_ano_id FROM academico_test.TANO_LECTIVO
         WHERE FK_TESTABLECIMIENTO = v_est_new AND NOMBRE = v_nombre_ano AND ACTIVE = TRUE;
    END IF;
    ...
    PERFORM academico_test.fn_audit_declarar(
        p_pk_usuario_solicitante,
        format('Actualización del periodo académico %s - %s', v_nombre_ano, v_nombre_jornada),
        v_establecimiento_id);
...
CREATE OR REPLACE FUNCTION academico_test.fn_periodo_crear(
    ...
    p_fecha_limite_matricula date, p_fk_jornada bigint,
    p_hora_inicio time without time zone, p_hora_fin time without time zone,
    p_reserva academico_test.bool_sn DEFAULT 'S'::character varying, p_bloques_por_defecto bigint DEFAULT 0,
    p_fk_periodo_anterior bigint DEFAULT NULL::bigint,
    p_descanso_inicio time without time zone[] DEFAULT NULL::time without time zone[],
    p_descanso_fin time without time zone[] DEFAULT NULL::time without time zone[],
    p_pk_usuario_solicitante bigint DEFAULT NULL::bigint
) RETURNS bigint LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    IF v_ano_id IS NULL THEN
        SELECT PK_ANO_LECTIVO INTO v_ano_id FROM academico_test.TANO_LECTIVO
         WHERE FK_TESTABLECIMIENTO = v_establecimiento AND NOMBRE = v_nombre_ano AND ACTIVE = TRUE;
    END IF;
    ...
    PERFORM academico_test.fn_audit_declarar(
        p_pk_usuario_solicitante,
        format('Creación del periodo académico %s - %s', v_nombre_ano, v_nombre_jornada),
        v_establecimiento);
...
CREATE OR REPLACE FUNCTION academico_test.fn_periodo_eval_actualizar(
    p_pk bigint, p_codigo character varying DEFAULT NULL::character varying,
    p_nombre character varying DEFAULT NULL::character varying,
    p_abreviacion character varying DEFAULT NULL::character varying,
    p_fecha_inicio date DEFAULT NULL::date, p_fecha_fin date DEFAULT NULL::date,
    p_fk_estado bigint DEFAULT NULL::bigint, p_porcentaje numeric DEFAULT NULL::numeric,
    p_pk_usuario_solicitante bigint DEFAULT NULL::bigint
) RETURNS bigint LANGUAGE plpgsql AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION academico_test.fn_periodo_eval_crear(
    p_fk_periodo bigint, p_codigo character varying, p_nombre character varying,
    p_abreviacion character varying, p_fecha_inicio date, p_fecha_fin date, p_fk_estado bigint,
    p_porcentaje numeric DEFAULT NULL::numeric, p_pk_usuario_solicitante bigint DEFAULT NULL::bigint
) RETURNS bigint LANGUAGE plpgsql AS $$
DECLARE
...
DECLARE
    v_activo BOOLEAN;
    v_audit  VARCHAR(120) := p_pk_usuario_solicitante::VARCHAR;
    v_establecimiento_id BIGINT; v_nombre VARCHAR(200);
BEGIN
    IF NOT academico_test.fn_periodo_usuario_puede_gestionar(p_pk_usuario_solicitante) THEN
        RAISE EXCEPTION 'El usuario no tiene el nivel de permisos necesario para realizar esta accion'
            USING ERRCODE = '42501';
    ...
    SELECT ACTIVE, NOMBRE INTO v_activo, v_nombre
    ...
    PERFORM academico_test.fn_audit_declarar(
        p_pk_usuario_solicitante, format('Eliminación del periodo académico %s', COALESCE(v_nombre, p_pk_periodo::TEXT)),
        v_establecimiento_id);
