...
--
-- El area filtra en DOS niveles, no solo uno: el referente (por
-- TREFERENTE_CURRICULAR_AREA) y cada ENUNCIADO de nivel 1 (por
-- TREFERENTE_ENUNCIADO.FK_REFERENTE_CURRICULAR_AREA, NULL = aplica a todas).
-- Sin lo segundo, un referente multi-area devolvia a una unidad de Matematicas
-- los enunciados de Ciencias.
--
...
--
-- ACTIVE vs ESTADO: se exigen LOS DOS. ACTIVE es el borrado logico de la
-- fila; ESTADO ('A'/'I') es el estado de NEGOCIO que el usuario edita en la
-- pantalla del catalogo. Son independientes -- en el servidor de test los
-- referentes 25 y 28 estan ACTIVE = true con ESTADO = 'I' --, asi que filtrar
-- solo por ACTIVE ofrecia referentes que el usuario habia dado de baja.
--
...
--
-- OJO con el orden: el gate de alcance corre ANTES de comprobar que el grado
-- existe, asi que un grado INEXISTENTE responde 403, no 404 (comprobado sobre
-- el servidor de test). Es deliberado y no se invierte: al reves, el endpoint
-- diria a cualquiera que puede autenticarse cuales PK_TGRADO existen y cuales
-- no, que es justo lo que el alcance territorial trata de no filtrar. El
-- P0002 queda para el grado que existe y el usuario si alcanza.
--
-- Depende de: V22 (TGRADO/TASIGNATURA/TAREA/TAREA_ASIGNATURA), V214.1
    ...
    WITH candidatos AS (
        SELECT rc.PK_REFERENTE_CURRICULAR AS pk,
         ...
         WHERE rc.ACTIVE = TRUE
           -- ESTADO = 'A' ademas de ACTIVE: ACTIVE es el borrado logico y
           -- ESTADO el estado de NEGOCIO que edita el usuario (Activo /
           -- Inactivo). En el servidor de test hay referentes con
           -- ACTIVE = true y ESTADO = 'I'; sin este filtro se ofrecian al
           -- front como si aplicaran.
           AND rc.ESTADO = 'A'
...
COMMENT ON FUNCTION academico_test.fn_refcurr_por_grado_asignatura(BIGINT, BIGINT, BIGINT, INT)
    IS 'El/los referentes curriculares que aplican a un GRADO + ASIGNATURA, con el arbol completo, para pintar la unidad y la actividad ANTES de que exista la unidad (V255 hace lo mismo pero partiendo de una unidad ya creada). Recorrido: TGRADO -> FK_TNIVEL_ENSENANZA -> TREFERENTE_CURRICULAR_NIVEL (N:N, V212) -> referente. Devuelve un CONJUNTO ordenado por especificidad, no una fila: la relacion es N:N y forzar "uno solo" seria inventar un criterio que el modelo no tiene. La asignatura es un FILTRO por area (TREFERENTE_CURRICULAR_AREA contra TAREA_ASIGNATURA) con la semantica de V212 -- un referente SIN filas de area aplica a TODAS, no a ninguna --, y el area de la asignatura se resuelve con COALESCE(TASIGNATURA.FK_TAREA_ASIGNATURA, TAREA.FK_TAREA_ASIGNATURA) porque la primera es nullable en V22 y viene vacia en la mayoria de las filas sembradas. Sin asignatura no se filtra por area. Exige ACTIVE = true Y ESTADO = ''A'' (son independientes: ACTIVE es el borrado logico, ESTADO el estado de negocio que edita el usuario; hay filas con ACTIVE = true y ESTADO = ''I''). Se descartan los referentes cuya vigencia no cubre p_anio (default: anio en curso; ANIO_VIGENCIA_HASTA NULL = indefinida). Orden = preferencia: primero los acotados al area pedida, luego los universales, y dentro de cada grupo la vigencia mas reciente; aplica_a_area y especificidad van en la salida para que el front muestre el criterio en vez de confiar a ciegas en el orden. Trae enfoque + es_evaluativo y tipo_evaluacion (las dos reglas que deciden que secciones e instrumentos se habilitan, V214.2), nivel_1_etiqueta / nivel_2_etiqueta (como llama ESE referente a sus niveles -- rotula con esto, no con literales), instrumento / normatividad, niveles y areas del referente, y enunciados: [{pk, texto, fkReferenteCurricularArea, area, evidencias:[{pk, texto}]}], ACOTADO al area de la asignatura pedida: un enunciado con FK_REFERENTE_CURRICULAR_AREA NULL no esta amarrado a ninguna area y sale siempre, y la evidencia hereda el area de su padre (CHK_TREFENUNC_AREA_SOLO_NIVEL1), por eso solo se filtra el nivel 1. NO delega en fn_refcurr_listar/fn_refenunc_listar (V213) a proposito: gatean sobre el menu REFERENTES_CURRICULARES, hoy solo super admin. Gate VER sobre PLANEADOR + alcance territorial por el grado (V277). Un grado inexistente responde 403 y no 404, porque el gate de alcance corre antes que la comprobacion de existencia -- deliberado: al reves el endpoint filtraria que PK_TGRADO existen. El P0002 aplica al grado que existe y el usuario si alcanza, y a la asignatura informada que no existe. V278.';
...
-- La fila de public.query ya existe en las bases desplegadas y el INSERT de
-- arriba es ON CONFLICT DO NOTHING: no la habria tocado. Se reconcilia el
-- detail aparte (patron V253/V279). No-op al reaplicar.
UPDATE public.query q
   SET detail = 'V278 -- el referente curricular que le corresponde a un ?grado= (obligatorio) y ?asignatura= (opcional), con TODOS sus datos, para pintar la unidad y la actividad ANTES de crear la unidad (con unidad ya creada, usar GET /planeador/unidades/:ID/referente, V255). El referente NO se elige a mano: se deriva del grado -> nivel de ensenanza -> referente(s) de ese nivel, y este endpoint hace el recorrido en una sola llamada. Devuelve un ARREGLO ordenado por preferencia (la relacion referente<->nivel es N:N, puede haber varios): primero los acotados al area de la asignatura pedida, luego los universales, y dentro de cada grupo la vigencia mas reciente -- lee aplica_a_area y especificidad para mostrar el criterio en vez de asumir la primera fila a ciegas. ?asignatura= filtra por AREA (TREFERENTE_CURRICULAR_AREA): un referente sin areas declaradas aplica a TODAS, no a ninguna. ?anio= (default: anio en curso) descarta los referentes fuera de vigencia. Solo se devuelven los referentes ACTIVE con ESTADO = Activo: ACTIVE es el borrado logico y ESTADO el estado de negocio que se edita en el catalogo, y hay filas con uno sin el otro. Por cada referente: nombre, descripcion, enfoque_valor + es_evaluativo, tipo_evaluacion_valor + nombre (las dos reglas que deciden que secciones e instrumentos se habilitan en la actividad, ver GET /planeador/actividades/:ID/configuracion), nivel_1_etiqueta y nivel_2_etiqueta (como llama ESE referente a sus niveles: "Enunciado"/"Evidencia" en primaria, "Proposito"/"Evidencia" en preescolar -- rotula con esto, no con literales), instrumento, instrumento_info_adicional, normatividad, vigencia, niveles:[{pk,nombre}], areas:[{pk,nombre}] ([] = todas) y enunciados:[{pk, texto, fkReferenteCurricularArea, area, evidencias:[{pk, texto}]}], ACOTADO al area de la asignatura pedida: un enunciado con FK_REFERENTE_CURRICULAR_AREA NULL no esta amarrado a ninguna area y sale siempre, y la evidencia hereda el area de su padre (CHK_TREFENUNC_AREA_SOLO_NIVEL1), por eso solo se filtra el nivel 1. total_count via COUNT(*) OVER(). Gate VER sobre PLANEADOR + alcance territorial por el grado. Un grado inexistente devuelve 403, NO 404: el gate de alcance corre antes que la comprobacion de existencia, a proposito, porque al reves el endpoint filtraria que grados existen. El 404 (P0002) es para el grado que existe y el usuario si alcanza, y para una ?asignatura= que no existe.'
  FROM public.microservice m
 WHERE m.id_microservice = q.microservice_id
   AND m.serviceid       = 'eval-col'
   AND q.path_template   = '/planeador/referente-curricular'
   AND q.http_method     = 'GET'
   AND q.detail IS DISTINCT FROM 'V278 -- el referente curricular que le corresponde a un ?grado= (obligatorio) y ?asignatura= (opcional), con TODOS sus datos, para pintar la unidad y la actividad ANTES de crear la unidad (con unidad ya creada, usar GET /planeador/unidades/:ID/referente, V255). El referente NO se elige a mano: se deriva del grado -> nivel de ensenanza -> referente(s) de ese nivel, y este endpoint hace el recorrido en una sola llamada. Devuelve un ARREGLO ordenado por preferencia (la relacion referente<->nivel es N:N, puede haber varios): primero los acotados al area de la asignatura pedida, luego los universales, y dentro de cada grupo la vigencia mas reciente -- lee aplica_a_area y especificidad para mostrar el criterio en vez de asumir la primera fila a ciegas. ?asignatura= filtra por AREA (TREFERENTE_CURRICULAR_AREA): un referente sin areas declaradas aplica a TODAS, no a ninguna. ?anio= (default: anio en curso) descarta los referentes fuera de vigencia. Solo se devuelven los referentes ACTIVE con ESTADO = Activo: ACTIVE es el borrado logico y ESTADO el estado de negocio que se edita en el catalogo, y hay filas con uno sin el otro. Por cada referente: nombre, descripcion, enfoque_valor + es_evaluativo, tipo_evaluacion_valor + nombre (las dos reglas que deciden que secciones e instrumentos se habilitan en la actividad, ver GET /planeador/actividades/:ID/configuracion), nivel_1_etiqueta y nivel_2_etiqueta (como llama ESE referente a sus niveles: "Enunciado"/"Evidencia" en primaria, "Proposito"/"Evidencia" en preescolar -- rotula con esto, no con literales), instrumento, instrumento_info_adicional, normatividad, vigencia, niveles:[{pk,nombre}], areas:[{pk,nombre}] ([] = todas) y enunciados:[{pk, texto, fkReferenteCurricularArea, area, evidencias:[{pk, texto}]}], ACOTADO al area de la asignatura pedida: un enunciado con FK_REFERENTE_CURRICULAR_AREA NULL no esta amarrado a ninguna area y sale siempre, y la evidencia hereda el area de su padre (CHK_TREFENUNC_AREA_SOLO_NIVEL1), por eso solo se filtra el nivel 1. total_count via COUNT(*) OVER(). Gate VER sobre PLANEADOR + alcance territorial por el grado. Un grado inexistente devuelve 403, NO 404: el gate de alcance corre antes que la comprobacion de existencia, a proposito, porque al reves el endpoint filtraria que grados existen. El 404 (P0002) es para el grado que existe y el usuario si alcanza, y para una ?asignatura= que no existe.';
