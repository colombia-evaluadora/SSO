-- =============================================================================
-- Área/Asignatura — funciones consolidadas (última versión)
--
-- Generado: 2026-09-04
--
-- Documento de referencia de SOLO LECTURA. NO ejecutar. NO es una migración
-- Flyway y NO debe copiarse a postgres/migrations/. Su único propósito es
-- reunir en un solo archivo la versión vigente (más reciente) de cada
-- función del módulo Área (TAREA) / Asignatura (TASIGNATURA) / Énfasis
-- (TENFASIS), dado que muchas de ellas fueron redefinidas (CREATE OR REPLACE)
-- varias veces a lo largo de la historia de migraciones.
--
...
--
-- Verificación: para cada función se corrió
--   grep -rn "FUNCTION academico_test.<nombre>(" postgres/migrations/
-- y se confirmó que el archivo usado abajo es el de número V más alto que
-- define esa función en todo postgres/migrations/. No se encontraron
-- migraciones más nuevas que las indicadas en el mapeo original.
-- =============================================================================


-- =============================================================================
-- HELPERS DE GATE (alcance por menu/capability + scope) — CU-86e2w4xdt
-- =============================================================================

-- Fuente: V40__area_subject_module.sql
...
-- Fuente: V40__area_subject_module.sql (CU-86e2w4xdt)
...
-- Fuente: V40__area_subject_module.sql (CU-86e2w4xdt). Reemplaza la firma vieja
-- (BIGINT, BIGINT), que autorizaba por fn_periodo_usuario_puede_gestionar /
-- _puede_escribir (listas fijas de FK_TROL, eliminadas — DROP formal en V211).
-- Ahora es un wrapper de una linea sobre fn_assert_permiso_seccion (helpers
-- nuevos en 00-helpers-permisos.sql), menu 'PERIODOS_ACADEMICOS'. La firma
-- posicional vieja (usuario, EE) se conserva; sede/jornada/accion se agregan
-- AL FINAL con DEFAULT, asi que los call sites de 2 argumentos que quedan sin
-- tocar en este y otros módulos (grado/grupo, plan de estudio, horario,
-- asignacion academica, escala de valoracion, criterio de evaluacion) siguen
-- funcionando: heredan capability + scope de establecimiento, pero un usuario
-- de nivel 3 (ADMINISTRATIVOS_SEDES) no satisface el scope sin sede+jornada
-- y queda denegado (fallo seguro), igual que antes de este cambio.
...
-- Extension propia de este consolidado (NO viene de la rama origen — ahi el
-- gate de LECTURA de la cascada academica se dejo fuera de alcance a
-- proposito, ver docs/gate-permisos-por-menu-analysis.md). Pedido explicito:
-- hoy el unico control de que un docente no vea la configuracion completa
-- del periodo es que el front oculte el menu; via API directa, cualquier
-- TSEDE_USUARIO en la sede pasaba fn_periodo_usuario_puede_ver sin mirar la
-- capability del menu. fn_periodo_puede_ver agrega esa capa: version BOOLEAN
-- (no lanza) del mismo modelo capability + scope de fn_periodo_gate_escritura,
-- pero para la accion 'VER', pensada para el WHERE de los listados/reportes
-- (mismo patron que fn_matricula_puede_ver, V40, para el modulo Matricula).
-- Reemplaza a fn_periodo_usuario_puede_ver en los ~22 call sites propios de
-- este consolidado (01 a 10); fn_periodo_usuario_puede_ver NO se toca ni se
-- elimina, porque la siguen usando listados/reportes/matricula/PIGSE fuera de
-- este alcance (ver V211).
