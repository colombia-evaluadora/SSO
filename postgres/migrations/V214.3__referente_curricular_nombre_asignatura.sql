...
-- ---------------------------------------------------------------------------
-- PK_LISTA_VALOR es GENERATED BY DEFAULT AS IDENTITY (V22) y las filas del
-- dump base entraron con pk explicito, asi que la secuencia se quedo detras
-- del maximo y el primer INSERT sin pk choca (misma trampa que documenta V147
-- para TARCHIVO). Resincronizarla antes de insertar; es idempotente.
SELECT setval(pg_get_serial_sequence('academico_test.tlista_valor', 'pk_lista_valor'),
              GREATEST((SELECT COALESCE(MAX(PK_LISTA_VALOR), 0)
                          FROM academico_test.TLISTA_VALOR), 1));
...
BEGIN
    ...
    -- El duplicado por VALOR ya se descarto arriba, asi que un unique_violation
    -- aqui solo puede ser la pkey: la secuencia de identidad detras del maximo
    -- porque alguien inserto con pk explicito (un dump). Se resincroniza y se
    -- reintenta una vez; si vuelve a fallar, que suba.
    BEGIN
    ...
    EXCEPTION WHEN unique_violation THEN
        PERFORM setval(pg_get_serial_sequence('academico_test.tlista_valor', 'pk_lista_valor'),
                       GREATEST((SELECT COALESCE(MAX(PK_LISTA_VALOR), 0)
                                   FROM academico_test.TLISTA_VALOR), 1));
        INSERT INTO academico_test.TLISTA_VALOR
            (CATEGORIA, NOMBRE, VALOR, CREATED_BY, CREATED_AT, ACTIVE)
