...
SELECT e.NIT, e.NOMBRE, e.FK_TMUNICIPIO, 'BACKFILL_V259', TRUE
 ...
 WHERE e.ACTIVE = TRUE
   ...
   AND EXISTS (SELECT 1 FROM pigse.TMUNICIPIO m WHERE m.PK_TMUNICIPIO = e.FK_TMUNICIPIO)
   AND (
...
SELECT count(*) INTO v_skip_est
  FROM academico_test.TESTABLECIMIENTO e
 WHERE e.ACTIVE = TRUE
   AND NOT EXISTS (SELECT 1 FROM pigse.TMUNICIPIO m WHERE m.PK_TMUNICIPIO = e.FK_TMUNICIPIO);
