...
-- fn_periodo_crear
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION academico_test.fn_periodo_crear(p_fk_sede bigint, p_fk_estado bigint, p_fecha_inicio date, p_fecha_fin date, p_fecha_limite_matricula date, p_fk_jornada bigint, p_hora_inicio time without time zone, p_hora_fin time without time zone, p_reserva academico_test.bool_sn DEFAULT 'S'::character varying, p_bloques_por_defecto bigint DEFAULT 0, p_fk_periodo_anterior bigint DEFAULT NULL::bigint, p_descanso_inicio time without time zone[] DEFAULT NULL::time without time zone[], p_descanso_fin time without time zone[] DEFAULT NULL::time without time zone[], p_pk_usuario_solicitante bigint DEFAULT NULL::bigint)
 RETURNS bigint
 LANGUAGE plpgsql
AS $function$
DECLARE
