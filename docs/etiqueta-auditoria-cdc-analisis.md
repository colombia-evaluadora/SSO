...
|---|---|---|
| `PATCH /establecimientos/funcionarios/:ID` (`fn_fun_actualizar`) | 500 `function ... does not exist` | La llamada SQL del catálogo tiene ~20 parámetros menos que la firma real de la función. **Corrección post-verificación contra prod** (ver §14 de `etiqueta-cambios-por-funcion.md`): no es solo drift de "cuando se agregaron campos" — la firma de `dev` local y la de prod ya **difieren entre sí** en su último parámetro (`p_lista_permisos jsonb` en local vs `p_direccion character varying` en prod), y el catálogo no coincide con ninguna de las dos. Drift de tres puntas, no de dos. |
| `PUT /periodos-academicos/editar/:ID` (`fn_periodo_actualizar`) | 500 `function ... does not exist` | El catálogo pasa `DESCANSO_INICIO`/`DESCANSO_FIN` como argumentos posicionales. **Corrección post-verificación contra prod**: no es que el catálogo esté mal en el vacío — esos dos parámetros **siguen existiendo** en la función tal como corre hoy en prod (172.233.184.248); es la función de `dev` local la que quedó adelantada (los descansos se separaron a `fn_descanso_agregar`/`fn_descanso_eliminar` en `dev`, pero prod todavía no tiene ese refactor). El catálogo coincide con prod, no con `dev`. |
...
Queda como limitación real (no un problema resuelto): modo `DML`/`PROCEDURE` no reciben el wrap automático — el truco requiere que el SQL original sea una expresión que devuelve filas (usable como subconsulta en un `FROM`), y ni un `INSERT`/`UPDATE` directo ni un `CALL` lo son. Ninguna fila en modo DML/PROCEDURE tenía el wrap tampoco bajo V80, así que no es una regresión — pero si en el futuro se registra una función de escritura vía esos modos, no obtiene auditoría de contexto automática.
