...
La validación definitiva es la que corre el CI: **Postgres 16 vacío +
`flyway migrate` del historial completo**, y después reaplicar cada migración
nueva con `psql` para comprobar idempotencia. Se reprodujo en local con la
misma imagen y los mismos parámetros que `.github/workflows/ci.yml`.

- `flyway migrate` sobre base vacía: **174 migraciones aplicadas, v207**,
  exit 0.
- Reaplicación de las 19 migraciones nuevas: **todas OK**, sin filas
  duplicadas.

Estado del catálogo sobre esa base limpia, comparado con el servidor de test:

| Tabla | base limpia | servidor |
|---|---:|---:|
| `app` | 3 | 3 |
| `app_microservice` | 11 | 11 |
| `app_route` | 16 | 16 |
| `endpoint` | 122 | 122 |
| `endpoint_microservice` | 117 | 119 (2 excluidas a propósito) |
| `route` | 28 | 29 (1 fila duplicada en el servidor) |

`public.query` queda en 174 filas y no 260 porque esta rama solo trae las
migraciones de `dev`: los endpoints de referente curricular, asistencias y
matrícula los registran sus propias ramas.

> **Lección aprendida.** La primera versión de estas migraciones se validó
> aplicando el SQL contra una base *ya poblada*, y eso ocultó un error real:
> las migraciones por debajo de V110 listaban `cacheable` y
> `cache_ttl_seconds` en su `INSERT`, columnas que añade V110. Contra una base
> limpia, V75 falla con `42703`. Para este tipo de migración, la única
> validación que sirve es `flyway migrate` desde cero.
