...
> Referencia función por función de lo que cambió, en cada una de las 49+5 funciones `academico_test.fn_*` de escritura, para adoptar `fn_audit_declarar`. Complementa [`etiqueta-auditoria-cdc-analisis.md`](etiqueta-auditoria-cdc-analisis.md) (el porqué del diseño) y [`etiqueta-catalogo-funciones-fn.md`](etiqueta-catalogo-funciones-fn.md) (el catálogo original de candidatas). Este documento solo cubre, por función: **en qué posición del cuerpo se insertó la llamada** a `fn_audit_declarar`, y **si se le mandó información extra** (más allá de actor+etiqueta) y de dónde salió.
>
> **Nota sobre la numeración `V67`–`V78` de las secciones de abajo**: esos números **ya no existen como archivos de migración separados**. Se implementó originalmente así (una migración `Vxx__..._adoptan_audit_declarar.sql` por módulo, que hacía `CREATE OR REPLACE FUNCTION` de la función ya existente agregándole el `PERFORM`), pero eso significaba redefinir la función completa dos veces: una en su migración original y otra en la migración de auditoría. Se corrigió fusionando cada `PERFORM` directamente en la migración que **originalmente** define esa función (`V37`–`V59`), y borrando las migraciones `V67`–`V79` que quedaron redundantes. La numeración `V67`–`V78` se conserva **solo como título de sección** en este documento (es la referencia histórica de "en qué tanda se hizo cada grupo"); la tabla de la sección siguiente dice dónde vive cada cambio *hoy*. Verificado con una reconstrucción completa desde cero (imagen limpia de Postgres + Flyway real, las 61 migraciones aplican sin error) para confirmar que la fusión no perdió ni duplicó nada.

## Dónde vive cada cambio hoy

| Sección de este documento | Migración(es) donde vive hoy |
|---|---|
| `V67` — `fn_grado_*` | `V43__grade_module.sql` |
| `V68` — área / asignatura / grupo | `V40__area_subject_module.sql` (área/asignatura), `V43__grade_module.sql` (grupo) |
| `V69` — escalas | `V42__rating_scale_module.sql` |
| `V70` — periodo académico / evaluación | `V37__academic_period_module.sql` (periodo/descanso), `V38__evaluation_period_module.sql` (periodo de evaluación) |
| `V71` — criterio/plan/horario/asignación | `V39__promotion_criteria_module.sql`, `V41__evaluation_criteria_module.sql`, `V44__study_plan_module.sql`, `V45__schedule_module.sql`, `V46__academic_assignment_module.sql` |
| `V72` — est (soft-delete), catálogo PLAN, sede_usuario (1) | `V53__establishment_module.sql`, `V59__trol_list_add_and_sync_to_role.sql`, `V51__employee_module.sql` |
| `V73` — sed (soft-delete), sede_usuario (2), usu | `V52__campuse_module.sql`, `V51__employee_module.sql` |
| `V74` — sed (crear/actualizar), est (crear) | `V52__campuse_module.sql`, `V53__establishment_module.sql` |
| `V75` — est (actualizar) | `V53__establishment_module.sql` |
| `V76` — fun (actualizar) | `V51__employee_module.sql` |
| `V77` — fix de orden en `fn_plan_agregar` | `V44__study_plan_module.sql` (ya con el orden correcto desde el principio — no queda rastro de "primero mal, después bien") |
| `V78` — sincronización desde prod (8 funciones) | Las 4 revertidas/reemplazadas: `V37`, `V51`, `V53` (reversión de firma), `V42` (reemplazo de `fn_escala_bulk_delete`). Las 5 net-new (`fn_enfasis_actualizar` ×2, `fn_enfasis_soft_delete`, `fn_fun_baja_establecimiento`, `fn_fun_enlazar_establecimiento`): `V67__fn_nuevas_desde_produccion_enfasis_fun_baja_enlazar.sql` (nuevo, no tenían definición previa que fusionar). |
| `V79` — seed `TLISTA_VALOR` (`ESTADO_ESTABLECIMIENTO`/`ZONA`/`ESTADOPERIODO`) | **Eliminada, no se conservó en ningún lado** (decisión explícita) — un ambiente nuevo necesitará ese seed aparte o `fn_est_crear`/`fn_periodo_actualizar` fallarán por FK/validación de categoría hasta que se agregue. |
...
No se tocó ninguna función en esta pasada — es un mapeo de dónde saldría cada `sede_id`, para implementarlo en una migración nueva cuando se decida hacerlo.
...
- Migración `V79` (nueva, separada) agregaba 3 valores de catálogo (`TLISTA_VALOR`) que estas funciones necesitan y que el catálogo local no tenía **en absoluto**: la categoría `ESTADO_ESTABLECIMIENTO` (id `533` exacto, porque `fn_est_crear` lo trae hardcodeado como constante), la zona `id=216` ("Urbana y Rural", usada por la sede por defecto del REV4), y la categoría `ESTADOPERIODO` completa (antes ni existía localmente — el periodo semilla `PK=2` apuntaba, por error de seed, a un valor de la categoría `PLAN`, cosa que la validación estricta de la versión de prod ahora rechaza). **`V79` se eliminó** (ver "Dónde vive cada cambio hoy" arriba) — un ambiente nuevo va a necesitar ese seed por separado.

### Hallazgo adicional (verificación de reconstrucción limpia): 3 funciones sin ninguna migración que las defina

Al fusionar los `PERFORM` en las migraciones originales, se reconstruyó la base completa desde cero (Postgres limpio + Flyway real, las 61 migraciones) para confirmar que la fusión no perdía nada — comparando función por función contra la base local ya funcionando. Aparecieron 3 funciones que la base local **tiene pero ningún archivo de este repo define**, ninguna relacionada con el trabajo de etiquetas: `fn_fun_permisos_actualizar` (la usa el REV4 de `fn_est_crear` para los permisos de rector/secretaria, y `fn_sed_crear`), `fn_list_roles`, y `fn_resolver_establecimiento_unico` (la usa `fn_fun_enlazar_establecimiento` como *fallback* cuando no viene `p_fk_establecimiento`). Existen en prod también, así que no es un problema de datos — es drift de versionado puro: en algún momento se crearon directo contra la base (local y de producción) sin pasar por una migración. No se tocó nada de esto (fuera de alcance), pero un ambiente reconstruido desde cero con solo este repo tendrá esas 3 funciones **ausentes**, y `fn_est_crear`/`fn_sed_crear`/`fn_fun_enlazar_establecimiento` fallarán en tiempo de ejecución (no al crearse) si tocan esos caminos de código.
